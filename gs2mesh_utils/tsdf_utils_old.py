...
from gs2mesh_utils.helper_size import resize_rgbd_to_intrinsics
...
class TSDF:
    ...
    def run(self, visualize=False):
        ...
        print(visualize, self.args, "in tsdf")
        ...
        for camera_number, left_camera in enumerate(tqdm(self.renderer.left_cameras)):
            if camera_number % self.args.TSDF_dilate != 0:
                continue
            if camera_number not in valid or camera_number in skip:
                continue
    
            output_dir = self.renderer.render_folder_name(camera_number)
    
            # Fix image load and resize
            image = Image.open(os.path.join(output_dir, 'left.png')).convert("RGB")
            image = image.resize((1284, 836))
            ...
            if self.args.TSDF_use_occlusion_mask:
                occ_mask = np.load(os.path.join(output_dir, f'out_{self.model_name}', 'occlusion_mask.npy')).astype(bool)
                depth *= occ_mask
            ...
            if image.dtype != np.uint8 or len(image.shape) != 3 or image.shape[2] != 3:
                raise ValueError(f"RGB image must be HxWx3 uint8, but got shape {image.shape} and dtype {image.dtype}")
            if depth.dtype != np.float32 or len(depth.shape) != 2:
                raise ValueError(f"Depth must be HxW float32, but got shape {depth.shape} and dtype {depth.dtype}")
            ...
            rgbd_image = o3d.geometry.RGBDImage.create_from_color_and_depth(
                rgb_o3d, depth_o3d,
                depth_scale=self.args.TSDF_scale,
            ...
            if visualize:
                ...
                plt.show()
                print("Depth range:",
                      np.min(np.asarray(rgbd_image.depth)[np.asarray(rgbd_image.depth) > 0]),
                      np.max(np.asarray(rgbd_image.depth)))
    
            # Set correct intrinsics (matching COLMAP)
            intrinsics = o3d.camera.PinholeCameraIntrinsic()
            intrinsics.set_intrinsics(
                width=1284, height=836,
                fx=7440.7070999954158, fy=7440.7070999954158,
                cx=642, cy=418
            )
    ...
    def run_1(self, visualize=False):
        """
        ...
        for camera_number, left_camera in enumerate(tqdm(self.renderer.left_cameras)):
            ...
            if skip is not None and camera_number in skip:
                continue
            output_dir = self.renderer.render_folder_name(camera_number)
            image = np.array(Image.open(os.path.join(output_dir, 'left.png')).convert("RGB").resize((1284, 836)), dtype=np.uint8)

            # image = np.array(Image.open(os.path.join(output_dir, 'left.png'))).astype(np.uint8).resize(1284,836),
            depth = np.load(os.path.join(output_dir, f'out_{self.model_name}', 'depth.npy')).astype(np.float32)
            depth[np.isnan(depth)] = 0
            depth[np.isinf(depth)] = 0

            print(self.args,"Check TSDF_USE_Mask, check!!!!") 
            if  self.args.TSDF_use_mask:
                object_mask = (np.load(os.path.join(output_dir, 'left_mask.npy')).astype(bool))
                if  self.args.TSDF_invert_mask:
                    object_mask = ~object_mask
            ...
            intrinsics.set_intrinsics(
                width=1284,
                height=832,
                fx=7440.7070999954158,
                fy=7440.7070999954158,
                cx=642,
                cy=418
            )
            # rgbd_image = resize_rgbd_to_intrinsics("rgb.png", "depth.png", intrinsics)
